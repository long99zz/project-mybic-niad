  ...
  const handleNext = async () => {
    if (step < totalSteps) {
      ...
      try {
        ...
        const invoice_id = invoiceRes.data.invoice_id; // Child ID for other operations
        ...
        // Lưu thông tin đơn hàng vào sessionStorage để OrderSuccessPage hiển thị đúng
        const orderInfo = {
          ...
          insurance_start: formatDateToISOString(form.departureDate),
          insurance_end: formatDateToISOString(form.returnDate),
          status: "Chưa thanh toán",
          // Thông tin bổ sung
          departure_location: "Việt Nam",
          destination: form.destination,
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex justify-between mt-8">
              <button
                ...
                className="px-10 py-3 bg-white border border-gray-400 rounded-lg text-lg font-bold text-gray-800 hover:bg-gray-100 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Quay lại
              </button>
              <button
                onClick={handleNext}
                disabled={isSubmitting}
                className="px-10 py-3 bg-red-600 text-white rounded-lg text-lg font-bold hover:bg-red-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSubmitting ? "Đang xử lý..." : (step === totalSteps ? "Xác nhận & Thanh toán" : "Tiếp tục")}
              </button>
            </div>
          </div>
        </div>
